...
export default function Footer() {
  return (
    <footer className="bg-cream border-t border-maroon/10">
      <div className="mx-auto max-w-6xl px-6 sm:px-10 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          <div>
            <h3 className="text-sm font-black uppercase tracking-wider text-pink-dark mb-4">
              Service
            </h3>
            ...
            <ul className="space-y-3">
              {socialLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="inline-flex items-center gap-2.5 text-sm text-maroon hover:text-maroon-light transition-colors"
                  >
                    {link.icon}
                    {link.label}
                  </Link>
      ...
      <div className="border-t border-maroon/10 px-6 sm:px-10 py-5">
        <p className="text-xs sm:text-sm text-maroon/70">
          © {new Date().getFullYear()}, Linen & Bloom. All rights reserved.
        </p>
      </div>
    </footer>
  );
